...
def select_image_model(models):
    if not models:
        return None
    priorities = ("dall-e-3", "gpt-image", "image")
...
def get_model_list(api_base_url, headers):
    last_error = None
    for path in MODEL_PATHS:
        endpoint = api_base_url.rstrip("/") + path
        try:
            if requests is not None:
                response = requests.get(endpoint, headers=headers, timeout=MODEL_DETECT_TIMEOUT_SECONDS)
                if response.status_code == 404:
                    last_error = RuntimeError(f"404 - {response.text}")
                    continue
                if response.status_code >= 400:
                    last_error = RuntimeError(f"{response.status_code} - {response.text}")
                    print(f"DEBUG model endpoint failed: {endpoint}; error={last_error}", flush=True)
                    continue
                models = extract_model_ids(response.json())
            else:
                request = UrlRequest(endpoint, headers=headers, method="GET")
                try:
                    with urlopen(request, timeout=MODEL_DETECT_TIMEOUT_SECONDS) as response:
                        models = extract_model_ids(json.loads(response.read().decode("utf-8")))
                except HTTPError as exc:
                    body = exc.read().decode("utf-8", errors="replace")
                    if exc.code == 404:
                        last_error = RuntimeError(f"404 - {body}")
                        continue
                    last_error = RuntimeError(f"{exc.code} - {body}")
                    print(f"DEBUG model endpoint failed: {endpoint}; error={last_error}", flush=True)
                    continue
            if models:
                return models
            ...
            last_error = exc
            print(f"DEBUG model endpoint failed: {endpoint}; error={exc}", flush=True)
    if last_error:
        print(f"DEBUG model detection failed: {last_error}", flush=True)
    return []


def detect_available_model(api_base_url=None):
    configured_model = get_config("image_model")
    if configured_model:
        print(f"DEBUG selected image model: {configured_model} (from app_config.json)", flush=True)
        return configured_model, [configured_model]

    api_base_url = api_base_url or load_api_base_url()
    if not api_base_url:
        print("DEBUG selected image model: none (api_base_url not configured)", flush=True)
        return None, []

    headers = local_api_headers()
    models = get_model_list(api_base_url, headers)
    print(f"DEBUG available models: {models}", flush=True)
    selected = select_image_model(models)
    if selected:
        set_config("image_model", selected)
        print(f"DEBUG selected image model: {selected}", flush=True)
    else:
        print("DEBUG selected image model: none, Cockpit default will be used", flush=True)
    return selected, models


def local_api_headers(image_model=None):
    access_token = get_access_token()
    headers = {"Content-Type": "application/json"}
    if image_model:
        headers["X-Codex-Model"] = image_model
    if access_token:
...
def summarize_image_payload(payload):
    prompt = str(payload.get("prompt") or "")
    reference_fields = []
    reference_preview = ""

    for key in ("image", "reference_image", "input_image"):
        value = payload.get(key)
        if isinstance(value, dict) and value.get("b64_json"):
            reference_fields.append(key)
            if not reference_preview:
                reference_preview = value.get("b64_json", "")[:20]

    images = payload.get("images")
    if isinstance(images, list):
        for image in images:
            if isinstance(image, dict) and image.get("b64_json"):
                reference_fields.append("images")
                if not reference_preview:
                    reference_preview = image.get("b64_json", "")[:20]
                break

    return {
        "prompt_prefix": prompt[:100],
        "reference_fields": reference_fields,
        "reference_b64_prefix": reference_preview,
...
def model_payload_error(status_code, body):
    lowered = (body or "").lower()
    return status_code == 400 and ("model" in lowered or "模型" in body or "可用模型" in body)


def payload_without_model(payload):
...
def post_local_image_api_with_model_fallback(api_base_url, payload, headers, model_candidates=None):
    try:
        ...
        tried = {payload.get("model")} if payload.get("model") else set()
        last_exc = first_exc
        for model in model_candidates or []:
            if not model or model in tried:
                continue
            retry_payload = {**payload, "model": model}
            retry_headers = local_api_headers(model)
            print(f"DEBUG retrying image request with alternate model: {model}", flush=True)
            try:
                return post_local_image_api(api_base_url, retry_payload, retry_headers)
            except Exception as exc:
                last_exc = exc
                print(f"DEBUG alternate image model failed: {model}; error={exc}", flush=True)
        raise last_exc
...
def post_local_image_api(api_base_url, payload, headers):
    ...
    for path in LOCAL_IMAGE_PATHS:
        ...
        print(f"DEBUG image request Payload summary: {json.dumps(summarize_image_payload(payload), ensure_ascii=False)}", flush=True)
        try:
            if requests is not None:
                response = requests.post(endpoint, json=payload, headers=headers, timeout=HTTP_IMAGE_TIMEOUT_SECONDS)
                if response.status_code == 404:
                    print(f"Cockpit response body: {response.text}", flush=True)
                    print(
                        f"Cockpit request payload summary: {json.dumps(summarize_image_payload(payload), ensure_ascii=False)}",
                        flush=True,
                    )
                    last_error = RuntimeError(f"404 - {response.text}")
                ...
                if response.status_code >= 400:
                    ...
                    if payload.get("model") and model_payload_error(response.status_code, response.text):
                        header_only_payload = payload_without_model(payload)
                        headers_without_model = dict(headers)
                        headers_without_model.pop("X-Codex-Model", None)
                        print(
                            "DEBUG Cockpit rejected payload model, retrying without model",
                            flush=True,
                        )
                        print(
                            f"DEBUG no-model payload summary: {json.dumps(summarize_image_payload(header_only_payload), ensure_ascii=False)}",
                            flush=True,
                        )
                        response = requests.post(
                            endpoint,
                            json=header_only_payload,
                            headers=headers_without_model,
                            timeout=HTTP_IMAGE_TIMEOUT_SECONDS,
                        )
                        if response.status_code == 404:
                            print(f"Cockpit response body: {response.text}", flush=True)
                            last_error = RuntimeError(f"404 - {response.text}")
                            print(f"DEBUG endpoint returned 404 after header-only retry, trying next path: {path}", flush=True)
                            continue
                        if response.status_code >= 400:
                            print(f"Cockpit response body: {response.text}", flush=True)
                            print(
                                f"Cockpit request payload summary: {json.dumps(summarize_image_payload(header_only_payload), ensure_ascii=False)}",
                                flush=True,
                            )
            ...
            try:
                ...
                body = exc.read().decode("utf-8", errors="replace")
                print(f"Cockpit response body: {body}", flush=True)
                print(
                    f"Cockpit request payload summary: {json.dumps(summarize_image_payload(payload), ensure_ascii=False)}",
                    flush=True,
                )
                if exc.code == 404:
                    ...
                    print(f"DEBUG endpoint returned 404, trying next path: {path}", flush=True)
                    continue
                if payload.get("model") and model_payload_error(exc.code, body):
                    header_only_payload = payload_without_model(payload)
                    headers_without_model = dict(headers)
                    headers_without_model.pop("X-Codex-Model", None)
                    print(
                    ...
                    header_only_request = UrlRequest(
                        endpoint,
                        data=json.dumps(header_only_payload).encode("utf-8"),
                        headers=headers_without_model,
                        method="POST",
                    )
                    try:
                        with urlopen(header_only_request, timeout=HTTP_IMAGE_TIMEOUT_SECONDS) as response:
                            return json.loads(response.read().decode("utf-8"))
                    except HTTPError as retry_exc:
                        retry_body = retry_exc.read().decode("utf-8", errors="replace")
                        print(f"Cockpit response body: {retry_body}", flush=True)
                        print(
                            f"Cockpit request payload summary: {json.dumps(summarize_image_payload(header_only_payload), ensure_ascii=False)}",
                            flush=True,
                        )
                        if retry_exc.code == 404:
                            last_error = RuntimeError(f"404 - {retry_body}")
                            print(f"DEBUG endpoint returned 404 after header-only retry, trying next path: {path}", flush=True)
                            continue
                        raise RuntimeError(f"{retry_exc.code} - {retry_body}") from retry_exc
...
def reference_payload(reference_image_path):
    ...
    note = (
        f"\nReference image provided locally: {path.name}.\n"
        "CRITICAL: The generated image MUST follow the EXACT composition, product angle, lighting setup, "
        "and camera framing from this reference image. Do NOT change the product pose, orientation, or framing. "
        "Only change the product color and decorative elements as specified in the prompt."
    )
...
def parse_percent(value, default=10, minimum=1, maximum=50):
...
def logo_position_xy(position, base_size, logo_size, margin):
    base_width, base_height = base_size
    logo_width, logo_height = logo_size
    normalized = str(position or "左上").strip()
    if normalized == "居中":
        return (base_width - logo_width) // 2, (base_height - logo_height) // 2
    if normalized == "右下":
        return max(base_width - logo_width - margin, 0), max(base_height - logo_height - margin, 0)
    return margin, margin


def add_logo_to_image(image_file, logo_path, output_file, opacity=0.82, position="左上", scale_percent=10):
    resolved_logo = resolve_logo_path(logo_path)
    if not resolved_logo.exists():
        print(f"Logo 文件不存在，跳过叠加: {resolved_logo}", flush=True)
        return False
    if resolved_logo.suffix.lower() != ".png":
        print(f"Logo 只支持 PNG 透明背景，跳过叠加: {resolved_logo}", flush=True)
        return False
    base = Image.open(image_file).convert("RGBA")
    logo = Image.open(resolved_logo).convert("RGBA")
    scale = parse_percent(scale_percent, default=10) / 100
    target_width = max(1, min(int(base.width * scale), 150))
    target_height = max(1, int(logo.height * (target_width / logo.width)))
    if target_height > 150:
        target_height = 150
        target_width = max(1, int(logo.width * (target_height / logo.height)))
    logo = logo.resize((target_width, target_height), Image.LANCZOS)
    if opacity < 1:
        alpha = logo.getchannel("A")
        alpha = alpha.point(lambda value: int(value * opacity))
        logo.putalpha(alpha)
    margin = max(20, int(min(base.width, base.height) * 0.02))
    x, y = logo_position_xy(position, (base.width, base.height), logo.size, margin)
    base.alpha_composite(logo, (x, y))
    base.convert("RGB").save(output_file)
    print(
        "DEBUG logo overlay:",
        f"logo={resolved_logo}",
        f"position={position or '左上'}",
        f"scale={scale_percent or 10}%",
        f"xy=({x},{y})",
        f"size={logo.size}",
        flush=True,
    )
...
def run_image_for_task(task):
    ...
    try:
        ...
        if should_add_logo:
            ...
            if not logo_path:
                print("Logo 文件不存在，跳过叠加", flush=True)
            elif add_logo_to_image(
                main_file,
                logo_path,
                logo_file,
                position=task.get("logo_position") or "左上",
                scale_percent=task.get("logo_scale") or 10,
            ):
...
def task_backfill_values(row):
    ...
    return {
        "SKU": data.get("sku") or "",
        "是否添加品牌logo": data.get("add_logo") or "",
        "logo文件地址": data.get("logo_path") or "",
        "logo位置": data.get("logo_position") or "左上",
        "logo缩放": data.get("logo_scale") or 10,
        "需校验品牌文字/元素": data.get("brand_check") or "",
        "优化后提示词": data.get("optimized_prompt") or "",
        "AI推荐标题": data.get("ai_title") or "",
        "生成时间": data.get("detail_generated_at") or data.get("generated_at") or "",
        "输出文件地址": task_output_path_text(row),
        "生成图片张数": len(image_files_for_task(dict(row), "all")),
        "图片颜色列表": data.get("color_name") or "",
        "执行状态": export_generation_status(data.get("status")),
...
PROGRAM_EXCEL_COLUMNS = [
    "SKU", "是否添加品牌logo", "logo文件地址", "logo位置", "logo缩放", "需校验品牌文字/元素", "优化后提示词",
...
def excel_task_from_row(row_index, data):
    ...
    return {
        ...
        "logo_position": str(data.get("logo位置") or "左上").strip() or "左上",
        "logo_scale": parse_percent(data.get("logo缩放"), default=10),
...
def generate_ai_title(task):
    try:
        ...
        cmd = [
            codex_path if Path(codex_path).exists() else "codex",
            "exec",
            "--skip-git-repo-check",
            prompt,
        ]
...
def auto_fix_codex_cli_model_config():
    target_model = "gpt-5.3-codex"
    unsupported_marker = "gpt-5.4-mini"
    config_path = Path(os.path.expandvars(r"%USERPROFILE%\.codex\config.toml"))
    if "%" in str(config_path):
        config_path = Path.home() / ".codex" / "config.toml"

    print(f"DEBUG codex config path: {config_path}", flush=True)
    try:
        config_path.parent.mkdir(parents=True, exist_ok=True)
        existed = config_path.exists()
        original_text = config_path.read_text(encoding="utf-8") if existed else ""
        lines = original_text.splitlines()
        model_pattern = re.compile(r'^(\s*model\s*=\s*)(["\'])(.*?)(\2)(.*)$')
        changed = False
        found_model = False
        old_model = None
        new_lines = []
        ...
        for line in lines:
            ...
            if match and not stripped.startswith("#"):
                found_model = True
                current_model = match.group(3)
                if unsupported_marker in current_model:
                    old_model = current_model
                    line = f'{match.group(1)}"{target_model}"{match.group(5)}'
                    changed = True
        ...
        if changed:
            if existed:
                shutil.copy2(config_path, config_path.with_name("config.toml.bak"))
            config_path.write_text("\n".join(new_lines) + "\n", encoding="utf-8")
            print(f"DEBUG codex config model: {old_model} → {target_model} (已修改)", flush=True)
            print(
                "[自动修复] Codex CLI 模型已从 gpt-5.4-mini 改为 gpt-5.3-codex，请重启 Cockpit 让配置生效。",
                flush=True,
            )
        else:
            print(f"DEBUG codex config model: {old_model or '未配置'} (无需修改)", flush=True)
    except Exception as exc:
        print(f"WARNING: 自动检查/修复 Codex CLI 配置失败：{exc}", flush=True)


def find_running_cockpit_paths():
    names = ("cockpit",)
    paths = []
    try:
        try:
        ...
        if psutil is not None:
            for proc in psutil.process_iter(["name", "exe"]):
                ...
                if exe and any(marker in name for marker in names):
                    paths.append(Path(exe))
            return paths
    except Exception as exc:
        print(f"DEBUG Cockpit process scan via psutil failed: {exc}", flush=True)
    ...
    try:
        ...
        print(f"DEBUG Cockpit process scan via wmic failed: {exc}", flush=True)
    return paths


def cockpit_scan_roots():
    roots = []
    templates = [
        r"%APPDATA%\Cockpit",
        r"%APPDATA%\cockpit",
        ...
        r"%LOCALAPPDATA%\cockpit",
        r"%USERPROFILE%\.cockpit",
        r"%USERPROFILE%\.codex",
    ]
    for template in templates:
        expanded = Path(os.path.expandvars(template))
        if "%" not in str(expanded) and expanded not in roots:
            roots.append(expanded)

    env_candidates = [
        os.environ.get("COCKPIT_HOME"),
        os.environ.get("COCKPIT_PATH"),
        os.environ.get("COCKPIT_DIR"),
    ]
    ...
    for exe_path in find_running_cockpit_paths():
        print(f"DEBUG Cockpit process path: {exe_path}", flush=True)
        install_dir = exe_path.parent
        if install_dir not in roots:
            roots.append(install_dir)
    return roots


def backup_path_for_config(path):
...
def auto_fix_cockpit_model_config():
    unsupported_model = "gpt-5.4-mini"
    target_model = "gpt-5.3-codex"
    config_extensions = {".json", ".toml", ".yaml", ".yml", ".ini", ".conf"}
    max_file_size = 5 * 1024 * 1024
    changed_files = []
    scanned_files = 0

    roots = cockpit_scan_roots()
    print("DEBUG Cockpit config scan roots:", [str(path) for path in roots], flush=True)

    for root in roots:
        if not root.exists():
            print(f"DEBUG Cockpit config scan skip missing: {root}", flush=True)
            continue
        if root.is_file():
            candidates = [root]
        else:
            try:
                ...
                print(f"DEBUG Cockpit config scan failed: {root}; error={exc}", flush=True)
                continue
        for path in candidates:
            try:
                ...
                if unsupported_model not in text:
                    continue
                backup = backup_path_for_config(path)
                shutil.copy2(path, backup)
                path.write_text(text.replace(unsupported_model, target_model), encoding="utf-8")
                changed_files.append(path)
                print(
                    f"[自动修复] 在 {path} 中发现 {unsupported_model} 并修改为 {target_model}",
                    flush=True,
                )
            except Exception as exc:
                print(f"DEBUG Cockpit config file skipped: {path}; error={exc}", flush=True)

    if not changed_files:
        print(
            "[扫描失败] 未在常见路径找到 gpt-5.4-mini 配置。该模型可能是 Cockpit 内部硬编码或存储在非标准位置。"
            " 请检查 Cockpit 界面设置，或联系 Cockpit 支持。",
            flush=True,
        )
    else:
        print(f"DEBUG Cockpit config scan modified files: {[str(path) for path in changed_files]}", flush=True)
    print(f"DEBUG Cockpit config scanned files: {scanned_files}", flush=True)


def main():
    auto_fix_codex_cli_model_config()
    auto_fix_cockpit_model_config()
